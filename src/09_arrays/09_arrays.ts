...
let scores: Array<number> = [8.5, 9.0, 10];
...
const mixedData: (string | number)[] = ["Haruto", 2804, "STU", 2026];
...
const apiConfig: readonly string[] = ["https://api.v1", "https://api.v2"];
// apiConfig.push("new_link"); // ERROR: Property 'push' does not exist on type 'readonly string[]'.

/* 10. ARRAYS TRONG DỰ ÁN THỰC TẾ (CINESTREAM & WEALTH TRACKER)               */
/* -------------------------------------------------------------------------- */

/**
 * Áp dụng Array kết hợp với Interface để quản lý danh sách dữ liệu phức tạp.
 */
